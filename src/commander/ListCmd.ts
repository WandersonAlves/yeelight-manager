...
import ListDevicesCase from '../modules/Discovery/ListDevices/ListDevicesCase';
...
export const ListCmd = async ({ verbose, debug, waitTime, colors }) => {
    ...
    if (colors) {
        ...
        Object.entries(ColorStorage.Colors).forEach(([colorName, colorCode]) => logger.info(`${colorName}: ${chalk.hex(colorCode)`${colorCode}`}`));
        process.exit(0);
    }
    await GetBindingFromContainer(ListDevicesCase).execute();
